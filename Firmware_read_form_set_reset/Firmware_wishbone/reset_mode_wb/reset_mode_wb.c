...
// ************************************************
//          *** Hardware Test Case for Program RESET ***
...
void main(void)
{
    ...
    print("[RESET_MODE_WB] Wishbone enabled\n");

    // -----------------------------
    // Performing Program RESET mode sequence
    // Converted from your cocotb firmware
    // -----------------------------

    print("[RESET_MODE_WB] Writing command 1: 0x00036472\n");
    REG32(NEURO_ADDR) = 0x00036472;
    wait_cycles(500);

    print("[RESET_MODE_WB] Writing command 2: 0x462B000B\n");
    REG32(NEURO_ADDR) = 0x462B000B;
    wait_cycles(500);

    print("[RESET_MODE_WB] Writing command 3: 0x44001405\n");
    REG32(NEURO_ADDR) = 0x44001405;
    wait_cycles(500);

    print("[RESET_MODE_WB] Writing command 4 RESET: 0x10088806\n");
    REG32(NEURO_ADDR) = 0x10088806;
    wait_cycles(500);

    print("[RESET_MODE_WB] Writing command 5 READ: 0x500888FF\n");
    ...
    // -----------------------------
    print("[RESET_MODE_WB] Reading back from 0x30000004\n");

    temp = read_wishbone(NEURO_ADDR);

    print("[RESET_MODE_WB] Readback value = ");
    ...
    print("[RESET_MODE_WB] test finished\n");
